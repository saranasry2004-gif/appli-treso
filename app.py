...
if page == "Dashboard":
    ...
    solde    = df["solde_cumule"].iloc[-1] if not df.empty else SOLDE_INITIAL
    entrees  = df[df["type"]=="entree"]["montant"].sum()
    sorties  = df[df["type"]=="sortie"]["montant"].sum()
    retards  = (ech["statut"]=="en_retard").sum()
    a_enc    = ech[ech["statut"]=="a_encaisser"]["montant"].sum()

    # ── Alerts ──────────────────────
    if solde < SEUIL_CRITIQUE:
        st.markdown(f'<div class="alert alert-danger">Alerte critique — Le solde actuel ({solde:,.0f} MAD) est inférieur au seuil critique de {SEUIL_CRITIQUE:,.0f} MAD. Intervention immédiate requise.</div>', unsafe_allow_html=True)
    elif solde < SEUIL_CRITIQUE * 2:
        st.markdown(f'<div class="alert alert-warning">Attention — Le solde actuel ({solde:,.0f} MAD) est proche du seuil critique. Surveillez les prochaines sorties.</div>', unsafe_allow_html=True)
    else:
        st.markdown(f'<div class="alert alert-success">Situation saine — Le solde actuel ({solde:,.0f} MAD) est au-dessus du seuil critique de {SEUIL_CRITIQUE:,.0f} MAD.</div>', unsafe_allow_html=True)
    if retards > 0:
        mont_ret = ech[ech["statut"]=="en_retard"]["montant"].sum()
        st.markdown(f'<div class="alert alert-warning">{retards} facture(s) client en retard — Montant total à récupérer : {mont_ret:,.0f} MAD. Relances à effectuer.</div>', unsafe_allow_html=True)
    ...
    <div class="kpi-grid">
      ...
      <div class="kpi-card success">
        ...
        <div class="kpi-sub">MAD · Période en cours</div>
      </div>
      <div class="kpi-card danger">
        <div class="kpi-label">Total Sorties</div>
        <div class="kpi-value red">{sorties:,.0f}</div>
        <div class="kpi-sub">MAD · Charges cumulées</div>
      </div>
      <div class="kpi-card neutral">
        <div class="kpi-label">A Encaisser</div>
        <div class="kpi-value blue">{a_enc:,.0f}</div>
    ...
    with col1:
        ...
        if not df.empty:
            ...
            ax.fill_between(df["date"], df["solde_cumule"],
                            alpha=0.12, color=C_TEAL)
            ax.plot(df["date"], df["solde_cumule"],
                    color=C_TEAL, linewidth=2, zorder=3)
    ...
    with col2:
        ...
        if not df.empty:
            ...
            e = gp.get("entree", pd.Series(0, index=gp.index))
            s = gp.get("sortie", pd.Series(0, index=gp.index))
            ax2.bar(x - w/2, e, width=w, color=C_GREEN, alpha=0.85, label="Entrées")
            ax2.bar(x + w/2, s, width=w, color=C_RED,   alpha=0.85, label="Sorties")
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    # Retards alert
    retards = df_ech[df_ech["statut"] == "en_retard"]
    if not retards.empty:
        st.markdown(
            f'<div class="alert alert-danger">{len(retards)} facture(s) en retard '
    ...
    ax.axhline(SEUIL_CRITIQUE, color=C_RED, linestyle="--",
               linewidth=1.3, label=f"Seuil critique ({SEUIL_CRITIQUE:,} MAD)", zorder=3)
    ...
    ax.legend(handles=[
        ...
        plt.Line2D([0],[0], color=C_RED, linestyle="--", linewidth=1.3, label="Seuil critique"),
        plt.scatter([],[], color=C_TEAL,   s=50, label="Solde satisfaisant"),
        plt.scatter([],[], color=C_ORANGE, s=50, label="Solde faible"),
        plt.scatter([],[], color=C_RED,    s=50, label="Alerte critique"),
